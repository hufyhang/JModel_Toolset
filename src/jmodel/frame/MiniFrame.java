...
public class MiniFrame extends JPanel {
    ...
    public void ready() {
        ...
        for(Node node : this.model.getNodes()) {
            ...
            // if node is a PointNode
            if(node.getClass() == PointNode.class) {
                fig = new PointNodeFigure((PointNode)node);
            }

            // if node is an ActorNode
            if(node.getClass() == ActorNode.class) {
                fig = new ActorNodeFigure((ActorNode)node);
            }
        ...
        for(Connect con : this.model.getConnects()) {
            ...
            // TODO: CONNECTOR TYPE
            connector = new LineConnector(con, con.getType());
    ...
    public void draw(Graphics g) {
        ...
        for(int index = 0; index != this.connectorLine.size(); ++index) {
            // set stroke style
            switch (this.model.getConnects().get(index).getType()) {
                case LINEAR:
            ...
            g2d.draw(this.connectorLine.get(index));
        ...
        for(Connector connector : this.connectors) {
            for(Shape end : connector.getEnds()) {
                g2d.draw(end);
            }
        }
